...
- Firefox: o Firefox nativo do Linux já cobre boa parte da verificação de motor Gecko (o Firefox para Windows/macOS renderiza de forma equivalente na imensa maioria dos casos relevantes para este site; a diferença de plataforma que mais importa aqui, fonte do sistema, já é neutralizada porque o site hospeda a própria fonte). O Firefox nativo usa compilação própria e o Playwright não dirige o Firefox de distribuição, então a suíte automática não passa por ele: a conferência do Firefox é manual, por captura de tela, feita pelo qa-engineer.
...
**Dois alvos Blink no Playwright, não um (ordem do líder, 22/09/2026).** `playwright.config.ts` roda a suíte de ponta a ponta inteira contra dois binários instalados nesta máquina: `blink` (chromium genérico, `/usr/bin/chromium-browser`) e `brave` (o navegador que o líder usa no dia a dia, `/usr/bin/brave-browser`), cada caminho configurável por variável de ambiente (`PLAYWRIGHT_CHROMIUM_PATH`, `PLAYWRIGHT_BRAVE_PATH`). Motivo: o Brave vem com bloqueador de conteúdo próprio ligado por padrão, capaz de esconder um recurso do site sem gerar erro visível nenhum — o tipo de falha que só aparece rodando neste navegador específico, nunca num Chromium genérico. Medido em 22/09/2026: as 13 verificações de `tests/e2e/` passam nos dois alvos, sem nenhuma divergência entre eles.

**Isto continua sendo cobertura de UM motor (Blink), agora em dois binários.** Não substitui Gecko (Firefox) nem WebKit (Safari), que seguem fora do automático e em conferência manual, conforme o resto desta seção. Dois sabores de Blink reduzem o risco de "passou no Chromium genérico mas quebra no navegador real do líder", mas não fecham a lacuna de motor nenhuma.
