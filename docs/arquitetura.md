...
**`MenuCurriculo`** desenha os três níveis (período, cadeira, unidade) como disclosures aninhados a partir do objeto `Curriculo`. Recebe o currículo e a resolução da rota atual; emite navegação. É o componente mais exigente em acessibilidade (seção 11) e o único que sabe renderizar o estado `em-breve`. Não carrega conteúdo de unidade nenhuma.
