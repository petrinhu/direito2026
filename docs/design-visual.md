...
Implementado em `unidade.html`: 5 citações reais no corpo do texto (Código de Processo Civil art. 319, na aba Resumo, bloco 3; e Código Civil art. 186, art. 927, Constituição Federal art. 5º incisos V e X, e Código de Processo Civil art. 319, II, na aba Petição comentada), um painel de demonstração com réplicas estáticas dos três estados ao final da aba Petição, e um apêndice "Dispositivos citados" que só existe na impressão.
...
| `scroll-margin-top` em todo alvo de âncora | `.bloco-resumo` e `.peca h3` em `unidade.html` | Não depende de suporte condicional: é uma propriedade isolada, e sua ausência só faz o navegador voltar ao comportamento de rolar a âncora até encostar no topo |
