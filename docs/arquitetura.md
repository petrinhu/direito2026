...
`vite-plugin-pwa` em modo `generateSW` (Workbox), com `registerType: 'autoUpdate'`.

**Política de atualização (troca de decisão, 23/09/2026, pedido do líder verbatim: "pode dar comando na página para o conteúdo ser recarregado sempre? Muita gente entra na página nova e só ve a antiga e reclama que não apareceu nada").** A decisão anterior era `registerType: 'prompt'`: o service worker novo ficava esperando TODAS as abas do site fecharem antes de assumir, e só então (com uma UI de aviso, nunca construída) o leitor decidiria atualizar. No celular, "todas as abas fecharem" quase nunca acontece, e é exatamente por isso que muita gente ficava presa na versão antiga vendo cache velho. A troca para `autoUpdate` liga `skipWaiting` e `clientsClaim` no service worker gerado (o plugin faz isso sozinho para este `registerType`, ver `vite.config.ts`): o service worker novo assume sozinho assim que termina de instalar, sem esperar nada, e passa a controlar as abas já abertas.

- **Quando a checagem acontece:** o navegador confere sw.js por conta própria em navegações completas e por um ciclo interno (até ~24h), mas isto é um SPA - o leitor pode ficar horas numa unidade sem disparar navegação nenhuma. Por isso `src/main.ts` também agenda checagens (`registration.update()`) de hora em hora, quando a aba volta a ficar visível, e na troca de rota (`src/app/atualizacaoSW/agendarVerificacoesAtualizacao.ts`). O momento escolhido para essas checagens é sempre um ponto de transição (aba estava em segundo plano, ou o leitor já está navegando para outro lugar), nunca no meio de uma leitura contínua; o recarregamento em si, quando a checagem encontra versão nova, é automático e embutido no plugin.
- **Quem está preso hoje:** a correção em si mora no service worker novo (a chamada a `self.skipWaiting()`/`clientsClaim()` está incondicional no `sw.js` gerado, não depende de nenhum código da página antiga pedir nada). Assim que o navegador de quem está preso hoje buscar esse `sw.js` novo - pela checagem própria dele ou por uma visita nova - o service worker novo assume sozinho e os próximos carregamentos de conteúdo daquela aba já vêm da versão nova, mesmo sem a aba dar reload cheio.
- **Módulo sob demanda apagado sob os pés da aba antiga:** uma aba aberta na versão A pode navegar para uma seção ainda não visitada e pedir, via `import()` dinâmico, um pedaço de código com hash da versão A que a publicação da B já removeu do servidor. O `.htaccess` devolve `index.html` no lugar (fallback de SPA), o módulo falha ao interpretar HTML como JavaScript, e a seção fica em branco sem aviso. O Vite emite `vite:preloadError` no `window` para este caso exato (documentação oficial, "Load Error Handling"); `src/main.ts` escuta o evento e recarrega a página uma vez, via `src/app/atualizacaoSW/recarregarAoFalharModulo.ts`, com uma marca em `sessionStorage` (janela de 10s) que impede um segundo recarregamento em laço se a mesma navegação disparar mais de uma falha em sequência. **Quem está preso hoje neste caso específico:** o código antigo não tem este listener (só chega com esta correção), então para essas pessoas vale o recarregamento manual uma única vez - a partir do momento em que a aba carregar a versão que já tem o listener, o problema nunca mais volta a exigir ação manual, nem nesta aba nem em nenhuma outra.
...
**Roteiro de verificação (QA, em navegador - fora do escopo desta correção, que só cobriu testes sem tela, L-50):**

1. Publicar a versão A, abrir o site numa aba, deixar em outra unidade. Publicar a versão B por cima. Sem nenhuma ação do leitor (sem F5 manual), conferir que a aba passa a servir a versão B - por exemplo, um texto ou dado que só existe na B aparecendo sozinho, dentro de uma hora (ou forçando a checagem: trocar de rota ou minimizar/restaurar a aba).
2. Publicar a versão A, abrir uma aba e deixá-la aberta numa seção qualquer (sem navegar). Publicar a versão B, que precisa remover pelo menos um pedaço de código com hash que só existia na A. Nessa mesma aba (ainda rodando o JavaScript da A), navegar para uma seção **ainda não visitada nesta sessão**. Conferir que a seção aparece com o conteúdo da B (não fica em branco, não precisa de F5 manual) - a página pode recarregar uma vez sozinha nesse meio-tempo, o que é esperado.
...
### O conserto não chegava a quem já visitou o site — cache sem impressão digital

**[FATO, achado do líder, medido no site JÁ PUBLICADO, 22/09/2026] mais grave que o defeito original, porque este já estava no ar.** Medição dele, contra o site publicado: `inter-500` respondia com o tamanho novo (16.472 B), mas `inter-400` (6.824 B), `inter-700` (7.000 B) e `lora-700` (8.004 B) continuavam respondendo com o tamanho **antigo**, do recorte quebrado — pedindo o mesmo endereço com um parâmetro qualquer na consulta (que ignora o cache), vinha o tamanho novo. O cabeçalho de resposta confirmava: acerto de cache com idade de milhares de segundos.

**Causa.** `public/.htaccess` marca `*.js|*.css|*.woff2|*.png|*.svg|*.webp` como `Cache-Control: public, max-age=31536000, immutable` — seguro **só** quando o nome do arquivo muda sempre que o conteúdo muda. As quatro fontes moravam em `public/assets/fontes/`, copiadas **cruas** pelo Vite (nome fixo, sem hash). Nome fixo + cache imutável de um ano: quem já tinha a fonte quebrada em cache ficaria com ela por até um ano, mesmo depois do conserto no repositório.

**Conserto.** As fontes passaram a morar em `src/ui/estilos/fontes/`, referenciadas por caminho relativo em `fontes.css`. O Vite trata isso como um asset importado pelo CSS: calcula o hash do conteúdo e escreve o nome final (`inter-400-<hash>.woff2`, por exemplo) tanto no arquivo publicado quanto na regra `@font-face`. O cache de um ano continua — agora é seguro, porque o nome só se repete se o conteúdo for byte a byte igual. `src/ui/layout/oferecimento-logo.png` recebeu o mesmo tratamento (era o único outro arquivo de `public/` nas mesmas condições com correção viável sem infraestrutura nova — ver exceção do favicon abaixo).

**Varredura do pacote inteiro (não só fontes), ordem do líder.** `scripts/verificar-cache-fingerprint.ts` lê `public/.htaccess` de verdade (nunca uma cópia hardcoded da lista de extensões — fonte única), reproduz a semântica do Apache (`<FilesMatch>` processados em ordem, `Header set` posterior substitui o anterior para o mesmo cabeçalho) para decidir o `Cache-Control` que cada arquivo de `dist/` receberia de fato, e reprova qualquer um que tenha `immutable` sem uma marca de versão plausível no nome (hífen + 8 caracteres do alfabeto base64url, o mesmo comprimento que Vite/Rollup e o pacote `workbox` já usam). Visto reprovando contra o estado real antes do conserto — 6 arquivos, medido: as 4 fontes, `oferecimento-logo.png` e `icones/favicon.svg` — e verde depois, com as fontes e o logo movidos para `src/` e hasheados.

**Exceção deliberada: `icones/favicon.svg`.** Referenciado por caminho **fixo** em dois lugares que não passam pelo empacotador — `index.html` (`<link rel="icon">`) e `public/manifest.webmanifest`, este último um arquivo estático copiado cru. Hashear o favicon exigiria gerar `manifest.webmanifest` como saída de build (um passo novo, para um arquivo pequeno que raramente muda) — desproporcional ao risco. Em vez disso, `public/.htaccess` ganhou um `<FilesMatch "^favicon\.svg$">` específico, depois do bloco genérico (a ordem importa: `Header set` posterior vence), com `Cache-Control: public, max-age=86400, must-revalidate` — um dia, não um ano, e **nunca** `immutable`. O portão não reprova mais este arquivo porque ele deixou de alegar imutabilidade sobre um nome que pode mudar, não porque ganhou um hash.

**O que acontece com quem já está com a fonte velha em cache (pergunta do líder).** Sim, o problema se resolve sozinho, sem nenhuma ação em cima do cache de ninguém: a página publicada nova referencia `inter-400-<hash-novo>.woff2` (nome diferente do que estava em cache), então o navegador de quem já visitou faz uma requisição nova para um endereço que nunca teve em cache — não há como servir o arquivo velho para esse endereço, porque ele nunca existiu com esse nome. O arquivo velho (`inter-400.woff2`, nome antigo) só continua em cache até expirar por conta própria (até um ano) ou até o navegador descartá-lo por espaço; como ninguém mais pede esse endereço, ele simplesmente fica sem uso, sem custo para o leitor.
...
| RI3 | Service worker mal configurado prende o leitor numa versão velha. | `registerType: 'autoUpdate'` (skipWaiting + clientsClaim incondicionais no service worker gerado, seção 9), `no-cache` em `index.html` e `sw.js`, checagem agendada em `src/main.ts`, e procedimento de limpeza documentado no runbook para o caso residual. |
